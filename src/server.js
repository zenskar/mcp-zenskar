...
function checkNeedsApproval(tool, args) {
  ...
  if (approval && approval.approved === true) {
    if (consumeApprovalToken(approval.token, tool.name)) {
      logger.info(
        `[${tool.name}] Approval token verified, executing with approved arguments`
      )
      ...
      if (modifiedArgs) {
        ...
        logger.info(
          `[${tool.name}] Using user-modified arguments:`,
          modifiedArgs
        )
      }
...
function validateToolArgs(toolName, args) {
  ...
  if (toolName === 'generateInvoice') {
    if (!Number.isInteger(args.from_date))
      errors.push(
        "'from_date' must be an INTEGER UNIX timestamp (seconds), not a date string."
      )
    if (!Number.isInteger(args.to_date))
      errors.push(
        "'to_date' must be an INTEGER UNIX timestamp (seconds), not a date string."
      )
    if (
      Number.isInteger(args.from_date) &&
      Number.isInteger(args.to_date) &&
