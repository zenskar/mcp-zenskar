...
import { Dim, StatusPill, fmtDate, fmtMoney, daysBetween, shortId } from './format';

type SortKey = 'invoice_number' | 'invoice_total' | 'amount_due' | 'due_date' | 'created_at';
...
export function InvoiceTable({ payload }: { payload: InvoiceTablePayload }) {
  ...
  const rows = useMemo(() => sortRows(payload.invoices, sortKey, sortDir), [payload.invoices, sortKey, sortDir]);
  ...
  const openInvoice = (r: InvoiceRow) => {
    if (!r.id) return;
    callHost('tools/call', { name: 'getInvoiceLineItems', arguments: { invoiceId: r.id } })
      .then(() => notifyHost('ui/message', { text: `Opened invoice ${r.invoice_number || r.id}.` }))
      .catch(() => notifyHost('ui/message', { text: `Run: getInvoiceLineItems invoiceId=${r.id}` }));
  };
    ...
    <div className="space-y-3">
      <header className="flex items-baseline justify-between gap-3">
        <h2 className="text-base font-semibold m-0">
          Invoices <span className="text-muted font-normal">({payload.total.toLocaleString()})</span>
          {payload.scope ? <span className="text-muted font-normal text-sm"> · {payload.scope}</span> : null}
        </h2>
        <span className="text-xs text-muted">click row → line items · headers sort</span>
      </header>
      ...
      <div className="overflow-auto rounded-md border border-border">
        <table className="w-full text-sm">
          <thead className="bg-row-hover text-xs uppercase tracking-wide text-muted">
            <tr>
              <Th>#</Th>
              <Th sortable active={sortKey === 'invoice_number'} dir={sortDir} onClick={() => toggle('invoice_number')}>Invoice</Th>
              <Th>Customer</Th>
              <Th>Period</Th>
              <Th>Status</Th>
              <Th align="right" sortable active={sortKey === 'invoice_total'} dir={sortDir} onClick={() => toggle('invoice_total')}>Total</Th>
              <Th align="right" sortable active={sortKey === 'amount_due'} dir={sortDir} onClick={() => toggle('amount_due')}>Due</Th>
              <Th sortable active={sortKey === 'due_date'} dir={sortDir} onClick={() => toggle('due_date')}>Due Date</Th>
          ...
          <tbody>
            {rows.length === 0 ? (
              <tr><td colSpan={10} className="py-8 text-center text-muted">No invoices match.</td></tr>
            ) : rows.map((r, i) => {
              const overdueDays = r.due_date && r.amount_due && r.amount_due > 0 ? daysBetween(r.due_date) : null;
              ...
              return (
                ...
                >
                  ...
                  <td className="px-3 py-2 font-mono text-xs">{r.invoice_number || <Dim>—</Dim>}</td>
                  ...
                  <td className="px-3 py-2 text-right tabular-nums">{fmtMoney(r.invoice_total, cur)}</td>
                  <td className={`px-3 py-2 text-right tabular-nums ${isOverdue ? 'text-[var(--color-status-overdue)] font-medium' : ''}`}>
                    {fmtMoney(r.amount_due, cur)}
                  </td>
                  <td className="px-3 py-2 whitespace-nowrap">{fmtDate(r.due_date)}</td>
                  <td className={`px-3 py-2 text-right tabular-nums ${isOverdue ? 'text-[var(--color-status-overdue)] font-medium' : 'text-muted'}`}>
...
function paginate(cursor: string | null | undefined) {
  if (!cursor) return;
  callHost('tools/call', { name: 'listInvoices', arguments: { cursor } })
    .catch(() => notifyHost('ui/message', { text: `Run: listInvoices cursor=${cursor}` }));
}
...
function sortRows(rows: InvoiceRow[], key: SortKey, dir: SortDir): InvoiceRow[] {
  ...
  const get = (r: InvoiceRow): string | number | null => {
    switch (key) {
      ...
      case 'invoice_total': return r.invoice_total;
      case 'amount_due': return r.amount_due;
