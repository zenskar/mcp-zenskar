...
export function AggregateTable({ payload }: { payload: AggregateTablePayload }) {
  const [sortKey, setSortKey] = useState<SortKey>('last_run_at');
  const [sortDir, setSortDir] = useState<SortDir>('desc');
  const rows = useMemo(() => sortRows(payload.aggregates, sortKey, sortDir), [payload.aggregates, sortKey, sortDir]);
  const toggle = (k: SortKey) => { if (k === sortKey) setSortDir(d => d === 'asc' ? 'desc' : 'asc'); else { setSortKey(k); setSortDir(k === 'name' ? 'asc' : 'desc'); } };
  const open = (r: AggregateRow) => fireTool('getAggregateById', { aggregateId: r.id }, `Opened aggregate ${r.name || r.id}.`);

  return (
    <div className="space-y-3">
      <header className="flex items-baseline justify-between gap-3">
        <h2 className="text-base font-semibold m-0">Aggregates <span className="text-muted font-normal">({payload.total.toLocaleString()})</span>
          {payload.scope ? <span className="text-muted font-normal text-sm"> · {payload.scope}</span> : null}</h2>
        <span className="text-xs text-muted">aka billable metrics</span>
      </header>
        ...
        <table className="w-full text-sm">
          <thead className="bg-row-hover text-xs uppercase tracking-wide text-muted">
            <tr>
              ...
              <Th>Unit</Th>
              <Th>Status</Th>
              <Th sortable active={sortKey === 'last_run_at'} dir={sortDir} onClick={() => toggle('last_run_at')}>Last Run</Th>
            </tr>
          </thead>
          <tbody>
            {rows.length === 0 ? (
              <tr><td colSpan={7} className="py-8 text-center text-muted">No aggregates match.</td></tr>
            ) : rows.map((r, i) => (
              <tr key={r.id || i} className="border-t border-border hover:bg-[var(--brand-row-hover)] cursor-pointer" onClick={() => open(r)}>
                ...
                <td className="px-3 py-2 font-mono text-xs">{r.datasource || <Dim>—</Dim>}</td>
                <td className="px-3 py-2 font-mono text-xs">{r.formula || <Dim>—</Dim>}</td>
                <td className="px-3 py-2 text-xs">{r.unit || <Dim>—</Dim>}</td>
                <td className="px-3 py-2"><StatusPill status={r.status} /></td>
                <td className="px-3 py-2 whitespace-nowrap">{fmtDate(r.last_run_at)}</td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
      <Pager cursor={payload.cursor} tool="listAggregates" />
    </div>
  );
}

function sortRows(rows: AggregateRow[], key: SortKey, dir: SortDir): AggregateRow[] {
  const mult = dir === 'asc' ? 1 : -1;
  const get = (r: AggregateRow): string | null => { switch (key) { case 'name': return r.name; case 'last_run_at': return r.last_run_at; case 'created_at': return r.created_at; } };
