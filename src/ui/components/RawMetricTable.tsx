...
export function RawMetricTable({ payload }: { payload: RawMetricTablePayload }) {
  const [sortKey, setSortKey] = useState<SortKey>('created_at');
  const [sortDir, setSortDir] = useState<SortDir>('desc');
  const rows = useMemo(() => sortRows(payload.raw_metrics, sortKey, sortDir), [payload.raw_metrics, sortKey, sortDir]);
  const toggle = (k: SortKey) => { if (k === sortKey) setSortDir(d => d === 'asc' ? 'desc' : 'asc'); else { setSortKey(k); setSortDir(k === 'name' ? 'asc' : 'desc'); } };
  const open = (r: RawMetricRow) => fireTool('getRawMetricById', { rawMetricId: r.id }, `Opened raw metric ${r.name || r.id}.`);

  return (
    <div className="space-y-3">
      <header className="flex items-baseline justify-between gap-3">
        <h2 className="text-base font-semibold m-0">Raw Metrics <span className="text-muted font-normal">({payload.total.toLocaleString()})</span>
          {payload.scope ? <span className="text-muted font-normal text-sm"> · {payload.scope}</span> : null}</h2>
        <span className="text-xs text-muted">click row → details</span>
      </header>
      <div className="overflow-auto rounded-md border border-border">
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.length === 0 ? (
              <tr><td colSpan={7} className="py-8 text-center text-muted">No raw metrics match.</td></tr>
            ) : rows.map((r, i) => (
              <tr key={r.id || i} className="border-t border-border hover:bg-[var(--brand-row-hover)] cursor-pointer" onClick={() => open(r)}>
                ...
                <td className="px-3 py-2 font-mono text-xs">{r.api_slug || <Dim>—</Dim>}</td>
                <td className="px-3 py-2 text-xs">{r.api_type || <Dim>—</Dim>}</td>
...
function sortRows(rows: RawMetricRow[], key: SortKey, dir: SortDir): RawMetricRow[] {
  const mult = dir === 'asc' ? 1 : -1;
  const get = (r: RawMetricRow): string | null => { switch (key) { case 'name': return r.name; case 'created_at': return r.created_at; } };
