...
import { Dim, StatusPill, fmtDate, fmtMoney } from './format';
...
export function PlanTable({ payload }: { payload: PlanTablePayload }) {
  ...
  const rows = useMemo(() => sortRows(payload.plans, sortKey, sortDir), [payload.plans, sortKey, sortDir]);
  const toggle = (k: SortKey) => { if (k === sortKey) setSortDir(d => d === 'asc' ? 'desc' : 'asc'); else { setSortKey(k); setSortDir(k === 'name' ? 'asc' : 'desc'); } };
  const open = (r: PlanRow) => fireTool('getPlanById', { planId: r.id }, `Opened plan ${r.name || r.id}.`);

  return (
    <div className="space-y-3">
      <header className="flex items-baseline justify-between gap-3">
        <h2 className="text-base font-semibold m-0">Plans <span className="text-muted font-normal">({payload.total.toLocaleString()})</span>
          {payload.scope ? <span className="text-muted font-normal text-sm"> · {payload.scope}</span> : null}</h2>
        <span className="text-xs text-muted">click row → details · headers sort</span>
      </header>
        ...
        <table className="w-full text-sm">
          <thead className="bg-row-hover text-xs uppercase tracking-wide text-muted">
            <tr>
              ...
              <Th>Status</Th>
              <Th>Currency</Th>
              <Th align="right" sortable active={sortKey === 'phase_count'} dir={sortDir} onClick={() => toggle('phase_count')}>Phases</Th>
          ...
          <tbody>
            {rows.length === 0 ? (
              <tr><td colSpan={8} className="py-8 text-center text-muted">No plans match.</td></tr>
            ) : rows.map((r, i) => (
              <tr key={r.id || i} className="border-t border-border hover:bg-[var(--brand-row-hover)] cursor-pointer" onClick={() => open(r)}>
                ...
                <td className="px-3 py-2 text-xs">{r.currency || <Dim>—</Dim>}</td>
                <td className="px-3 py-2 text-right tabular-nums">{r.phase_count ?? <Dim>—</Dim>}</td>
                <td className="px-3 py-2 text-right tabular-nums">{r.mrr ? fmtMoney(r.mrr.amount, r.mrr.currency) : <Dim>—</Dim>}</td>
      ...
      <Pager cursor={payload.cursor} tool="listPlans" />
    </div>
  );
}
...
function sortRows(rows: PlanRow[], key: SortKey, dir: SortDir): PlanRow[] {
  ...
  const get = (r: PlanRow): string | number | null => {
    switch (key) { case 'name': return r.name; case 'phase_count': return r.phase_count; case 'mrr': return r.mrr?.amount ?? null; case 'created_at': return r.created_at; }
  };
