...
const DEBUG = process.env.ZENSKAR_MCP_UI_DEBUG === 'true' || process.env.MCP_DEBUG === 'true';
function dbg(...args) {
...
export function wrapToolResponse(toolName, rawData, fallbackText, args) {
  ...
  let payload;
  try { payload = route.toPayload(rawData, args); }
  catch (err) {
...
function arrayLength(payload) {
  ...
  for (const k of ['customers', 'invoices', 'lines', 'payments', 'credit_notes', 'contracts', 'transactions', 'rows', 'items', 'data']) {
